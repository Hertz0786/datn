...
/// Full paginated list of a specific user's bookmarks. Opened from
/// the "See all" link in [FriendProfileScreen]. Bookmarks are private —
/// only the user themselves and their friends can view them, and the
/// backend enforces that with a 403.
class FriendBookmarksScreen extends StatefulWidget {
  const FriendBookmarksScreen({
    super.key,
  ...
  @override
  State<FriendBookmarksScreen> createState() => _FriendBookmarksScreenState();
}

class _FriendBookmarksScreenState extends State<FriendBookmarksScreen> {
  ...
  Future<void> _load() async {
    ...
    try {
      final FeedPage page = await PostsApi.instance.userBookmarks(
        widget.userId,
        limit: _pageSize,
      );
      if (!mounted) {
      ...
      setState(() => _loadError = 'Failed to load bookmarks: $error');
    } finally {
      if (mounted) {
  ...
  Future<void> _loadMore() async {
    ...
    try {
      ...
      // Silent: leave pagination state unchanged.
    } finally {
      if (mounted) {
  ...
  Widget build(BuildContext context) {
    final String title = widget.displayName.trim().isEmpty
        ? 'Bookmarks'
        : "${widget.displayName}'s bookmarks";
      ...
      return ListView(
        ...
        children: [
          ...
          const EmptyStateView(
            ...
            title: "Can't view bookmarks",
            message:
                "Bookmarks are private. You can only see them once you're friends.",
          ),
        ],
      );
    }
      ...
      return ListView(
        ...
        children: [
          ...
          const EmptyStateView(
            ...
            message: 'Posts your friend has saved will appear here.',
          ),
        ],
      );
    }
